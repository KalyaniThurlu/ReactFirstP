...
export function XmlHttpRequestComponent() {
    const [product, setProduct] = useState({ 
        title: '', price: 0, rating: { rate: 0, ratings: 0, reviews: 0 }, offers: [], image: '' });
    ...
    .then((response) => {
        if (response) {
            ...
            throw new Error('Failed to fetch data'); // Throw an error to be caught by .catch()
        }
    })
    .then((data) => {
        setProduct(data); // Assuming setProduct is a function to set the product state
    })
    .catch((error) => {
                    ...
                    <div className="mt-3">
                        ...
                        <ul className="list-unstyled">
                            {product.offers.map(offer => (
                                <li className="bi bi-tag-fill text-success my-2" key={offer}>
